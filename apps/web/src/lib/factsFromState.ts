...
/** Build engine TripFacts from in-memory workspace state (no DB round-trip). */
export function factsFromState(args: {
  trip: TripRow;
  ...
  const activeExpenses = args.expenses.filter((e) => !e.supersededById);
  return {
